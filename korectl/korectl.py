...
def load_yaml(path):
    try:
        ...
        raise SawectlValidationError(f"[ERROR] Invalid YAML format in {path}: {ye}")
    except Exception as e:
        print(f"[ERROR] Failed to load YAML from {path}: {e}")
        raise SawectlValidationError(f"[ERROR] Failed to load YAML from {path}: {e}")
...
def load_json_schema(schema_path):
    try:
        ...
        raise SawectlValidationError(f"[ERROR] Failed to load schema from {schema_path}: {e}")
...
def validate_against_schema(yaml_data, schema_path):
    ...
    try:
        ...
        raise SawectlValidationError(f"[SCHEMA FAIL] {e.message} at {list(e.path)}")
    except Exception as e:
        print(f"[SCHEMA ERROR] {e}")
        raise SawectlValidationError(f"[SCHEMA ERROR] {e}")
...
def extract_module_class_method(action_str, context_modules):
    # Support context.module.method as well
    ...
    if parts[0] == 'context' and len(parts) >= 3:
        ...
        module_name, class_name = module_ref.split(".")
        return module_name, class_name, method
    elif len(parts) == 3:
        return parts[0], parts[1], parts[2]
    elif len(parts) == 2:
        # 🟡 Ambiguous: treat module and class as same
...
def validate_step(step, modules_dir, context_modules):
    ...
    module_name, class_name, method_name = extract_module_class_method(action_str, context_modules)
    if not module_name or not class_name or not method_name:
        return False, f"Cannot resolve module/class/method in action: '{action_str}'"

    manifest = load_module_manifest(modules_dir, module_name)
    if manifest is None:
        return False, f"Module '{module_name}' not found or missing 'module.yaml'"

    if manifest.get("class") != class_name:
        return False, f"Class mismatch in module '{module_name}': expected '{class_name}', found '{manifest.get('class')}'"

    methods = manifest.get("methods", [])
    method_entry = next((m for m in methods if m["name"] == method_name), None)
    if not method_entry:
        return False, f"Method '{method_name}' not found in '{module_name}.{class_name}'. Available methods: [{', '.join(m['name'] for m in methods)}]"

    expected_args = {arg['name'] for arg in method_entry.get('arguments', []) if arg.get('required')}
    provided_args = set(step.get('input', {}).keys() if 'input' in step else step.get('config', {}).keys())
    missing = expected_args - provided_args

    if missing:
        return False, f"Missing required input(s) {missing} in step '{step['id']}'"
...
def validate_workflow_deep(args):
    ...
    if 'name' not in workflow or 'steps' not in workflow:
        ...
        raise SawectlValidationError("[FAIL] Workflow must contain 'name' and 'steps'")

    step_ids = set()
    for step in workflow['steps']:
        if step['id'] in step_ids:
            print(f"[FAIL] Duplicate step ID: {step['id']}")
            raise SawectlValidationError(f"[FAIL] Duplicate step ID: {step['id']}")
        step_ids.add(step['id'])

        ok, msg = validate_step(step, modules_dir, context_modules)
        if not ok:
            print(f"[FAIL] {msg}")
            raise SawectlValidationError(f"[FAIL] {msg}")
        elif args.verbose:
            print(f"[OK] {msg}")
    ...
    for cm_id, cm_conf in context_modules.items():
        ...
        if not ref:
            ...
            raise SawectlValidationError(f"[FAIL] Context module '{cm_id}' missing 'module'")
        module_name = ref.split('.')[0]
        manifest = load_module_manifest(modules_dir, module_name)
        if manifest is None:
            print(f"[FAIL] Context module type '{module_name}' not found")
            raise SawectlValidationError(f"[FAIL] Context module type '{module_name}' not found")
        elif args.verbose:
    ...
    if 'global_failure_handler' in workflow:
        ...
        if not ok:
            ...
            raise SawectlValidationError(f"[FAIL] global_failure_handler: {msg}")
        elif args.verbose:
    ...
    if 'on_failure' in workflow:
        for step in workflow['on_failure'].get('steps', []):
            ...
            if not ok:
                ...
                raise SawectlValidationError(f"[FAIL] on_failure step: {msg}")
            elif args.verbose:
    ...
    if 'on_success' in workflow:
        for step in workflow['on_success'].get('steps', []):
            ...
            if not ok:
                ...
                raise SawectlValidationError(f"[FAIL] on_success step: {msg}")
            elif args.verbose:
...
def validate_all_modules(args):
    ...
    if not modules_dir.exists():
        ...
        raise SawectlValidationError(f"[ERROR] Modules path '{modules_dir}' does not exist.")
    ...
    if any_fail:
        ...
        raise SawectlValidationError("[RESULT] ❌ One or more modules failed validation")
    else:
...
def run_workflow(args):
    try:
        res = requests.post(
            f"http://{args.server}/${args.workflow}",
        ...
        raise SawectlValidationError(f"[ERROR] Failed to trigger workflow: {e}")
...
def init_workflow(args):
    ...
    if args.full:
        ...
        if args.trigger not in valid_triggers:
            ...
            raise SawectlValidationError(f"[ERROR] Invalid trigger type '{args.trigger}'. Allowed: {', '.join(valid_triggers)}")
        ...
        if args.trigger not in valid_triggers:
            ...
            raise SawectlValidationError(f"[ERROR] Unsupported trigger type '{args.trigger}'. Must be one of: {', '.join(valid_triggers)}")
    ...
    header_comment = f"""# =============================================
# koreflow Full Workflow Example
# Generated via `sawectl init workflow --full`
# 
# 🔗 DSL Reference: https://github.com/yuribernstein/koreflow/docs/dsl.md
# 🔗 Modules Reference: https://github.com/yuribernstein/koreflow/docs/modulesdev.md
# 
# Modify this file to suit your use case.
# =============================================
"""
# Clean up top-level dummy fields if empty
    ...
    with open(path, 'w') as f:
        ...
        # Use safe_dump but prettify "steps" with blank lines manually
        dumped = yaml.dump(wf, sort_keys=False, width=120)

        # Add spacing between each step (or any top-level list item)
        def add_spacing_to_blocks(text, key):
            ...
            for i, line in enumerate(lines):
                if line.strip().startswith(f"- id:"):
                    if inside_block:
                        result.append("")  # insert blank line between items
                    inside_block = True
                result.append(line)
            return "\n".join(result)
...
def main():
    ...
    if getattr(args, 'help', False) or not hasattr(args, 'func'):
        ...
        koreflow CLI Tool — Workflow Automation for Humans.
        ...
        run                   Trigger an ad-hoc workflow against a running koreflow engine
        ...
        --server <host:port>         Address of the koreflow server (e.g., localhost:8080)
        ...
        Documentation → https://koreflow.dev/docs
        """)
        sys.exit(0)
    args.func(args)

def validate_workflow_from_file(wf_path, modules_dir="modules", verbose=False):
    import argparse
    import io
    import contextlib
    ...
    try:
        ...
        with contextlib.redirect_stdout(io.StringIO()) as out, contextlib.redirect_stderr(io.StringIO()) as err:
            message = err.getvalue() or out.getvalue() or str(e)
